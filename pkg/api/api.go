...
func setupRoutes(server *echo.Group, gcs *gcs.Client, tokenTeamMap map[string]string, logger *slog.Logger) {
	server.POST("/story", func(c echo.Context) error {
		...
		_, err = gcs.ReadFile(c.Request().Context(), fmt.Sprintf("datafortellinger/%v/%v", storyMeta.Slug, metadataFile))
		if err == nil {
		...
		if err := gcs.UploadFile(c.Request().Context(), fmt.Sprintf("datafortellinger/%v/%v", storyMeta.Slug, metadataFile), storyMetaBytes); err != nil {
	...
	server.PUT("/story/:id", func(c echo.Context) error {
		...
		if err := deleteStoryFiles(c.Request().Context(), gcs, fmt.Sprintf("datafortellinger/%v", storyMeta.Slug)); err != nil {
...
func uploadStoryFiles(c echo.Context, gcs *gcs.Client, storyMeta storyMetadata) error {
	...
	for fileName, fileHeader := range c.Request().MultipartForm.File {
		...
		if err := gcs.UploadFile(c.Request().Context(), fmt.Sprintf("datafortellinger/%v/%v", storyMeta.Slug, fileName), fileBytes); err != nil {
			return err
		}
	}
	return nil
